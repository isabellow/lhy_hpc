...
def convert_coords(raw_coords):
    ...
    Converts relative ML/AP values to absolute (lamda-oriented) values in mircons
    ...
    # adjust ml and convert to microns
    abs_ml = (rel_ap - rel_ml)*1000

    # adjust ap (TODO) and convert to microns
    abs_ap_approx = dmdl_rel2abs(rel_ap*1000)

    # account for known shank distance
    abs_ap = np.asarray([np.mean(abs_ap_approx)-75, np.mean(abs_ap_approx)+75])
...
def probe_to_brain(insert_coords, probe_depth, probe_coords):
    '''
    TODO fix angling - currently tip points away from midline
    ...
    brain_ap[A_idx] = A_tip_ap + probe_ap[A_idx]
    brain_ap[B_idx] = B_tip_ap + probe_ap[B_idx] - 150
